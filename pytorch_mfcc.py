...
class MFCC(torch.nn.Module):
    def __init__(self, samplerate=16000, winlen=512, winstep=512, numcep=13, nfilt=26, nfft=None, lowfreq=0,
    ...
    def forward(self, signals, lengths):
        ...
        for i in range(len(outs)):
            ...
            zeros = torch.zeros((max_len - outs[i].shape[0], outs[i].shape[1]), dtype=self.tensor_type).to(
                self.torch_device)
    ...
    def magspec(self, frames):
        ...
        if frames.shape[1] < self.nfft:
            fshape = frames.shape
            cat_zeros = torch.zeros([fshape[0], self.nfft - fshape[1]], dtype=self.tensor_type,
